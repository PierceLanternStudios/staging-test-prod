# Pierces Staging Tester Mc Here TypeScript API Library
...
This library provides convenient access to the Pierces Staging Tester Mc Here REST API from server-side TypeScript or JavaScript.
...
```sh
npm install git+ssh://git@github.com:stainless-sdks-staging/pierces-staging-tester-typescript.git
```
...
const response: PiercesStagingTesterMcHere.StoreListInventoryResponse = await client.store.listInventory();
```
...
const response = await client.store.listInventory().catch(async (err) => {
  if (err instanceof PiercesStagingTesterMcHere.APIError) {
...
1. Via the `PIERCES_STAGING_TESTER_MC_HERE_LOG` environment variable
...
const client = new PiercesStagingTesterMcHere({
  logger: logger.child({ name: 'PiercesStagingTesterMcHere' }),
...
const client = new PiercesStagingTesterMcHere({ fetch });
```
...
```ts
import PiercesStagingTesterMcHere from 'npm:pierces-staging-tester';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks-staging/pierces-staging-tester-typescript/issues) with questions, bugs, or suggestions.
