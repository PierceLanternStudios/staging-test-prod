...
This library provides convenient access to the Pierces Staging Tester REST API from server-side TypeScript or JavaScript.
...
const response: PiercesStagingTester.StoreListInventoryResponse = await client.store.listInventory();
```
...
const response = await client.store.listInventory().catch(async (err) => {
  if (err instanceof PiercesStagingTester.APIError) {
...
1. Via the `PIERCES_STAGING_TESTER_LOG` environment variable
...
const client = new PiercesStagingTester({
  logger: logger.child({ name: 'PiercesStagingTester' }),
...
const client = new PiercesStagingTester({ fetch });
```
...
```ts
import PiercesStagingTester from 'npm:pierces-staging-tester';
